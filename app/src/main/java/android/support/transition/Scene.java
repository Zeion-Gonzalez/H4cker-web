...
public class Scene {
    ...
    public static Scene getSceneForLayout(@NonNull ViewGroup sceneRoot, @LayoutRes int layoutId, @NonNull Context context) {
        SparseArray<Scene> scenes = (SparseArray) sceneRoot.getTag(C0104R.id.transition_scene_layoutid_cache);
        if (scenes == null) {
            scenes = new SparseArray<>();
            sceneRoot.setTag(C0104R.id.transition_scene_layoutid_cache, scenes);
        }
    ...
    public static void setCurrentScene(View view, Scene scene) {
        view.setTag(C0104R.id.transition_current_scene, scene);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public static Scene getCurrentScene(View view) {
        return (Scene) view.getTag(C0104R.id.transition_current_scene);
    }
