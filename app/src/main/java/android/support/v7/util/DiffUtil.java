    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (snake != null) {
                ...
                stack.add(left);
                if (!snake.reverse) {
                    range.oldListStart = snake.f36x + snake.size;
                    range.newListStart = snake.f37y + snake.size;
                } else if (snake.removal) {
                    range.oldListStart = snake.f36x + snake.size + 1;
                    range.newListStart = snake.f37y + snake.size;
                } else {
                    range.oldListStart = snake.f36x + snake.size;
                    range.newListStart = snake.f37y + snake.size + 1;
                }
                stack.add(range);
            } else {
                rangePool.add(range);
        ...
        for (int d = 0; d <= dLimit; d++) {
            for (int k = -d; k <= d; k += 2) {
                ...
                if (checkInFwd && k >= (delta - d) + 1 && k <= (delta + d) - 1 && forward[kOffset + k] >= backward[kOffset + k]) {
                    ...
                    outSnake.f36x = backward[kOffset + k];
                    outSnake.f37y = outSnake.f36x - k;
            ...
            for (int k2 = -d; k2 <= d; k2 += 2) {
                ...
                if (!checkInFwd && k2 + delta >= (-d) && k2 + delta <= d && forward[kOffset + backwardK] >= backward[kOffset + backwardK]) {
                    ...
                    outSnake2.f36x = backward[kOffset + backwardK];
                    outSnake2.f37y = outSnake2.f36x - backwardK;
    ...
    public static class DiffResult {
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                int endX = snake.f36x + snakeSize;
                int endY = snake.f37y + snakeSize;
                if (endX < posOld) {
                ...
                for (int i = snakeSize - 1; i >= 0; i--) {
                    if ((this.mOldItemStatuses[snake.f36x + i] & 31) == 2) {
                        batchingCallback.onChanged(snake.f36x + i, 1, this.mCallback.getChangePayload(snake.f36x + i, snake.f37y + i));
                    }
                }
                posOld = snake.f36x;
                posNew = snake.f37y;
