...
public class Yuv444pToYuv420p implements Transform {
    ...
    public void transform(Picture src, Picture dst) {
        ...
        if (this.shiftUp > this.shiftDown) {
            m2138up(dst.getPlaneData(0), this.shiftUp - this.shiftDown);
            m2138up(dst.getPlaneData(1), this.shiftUp - this.shiftDown);
            m2138up(dst.getPlaneData(2), this.shiftUp - this.shiftDown);
    ...
    /* renamed from: up */
    private void m2138up(int[] dst, int up) {
        for (int i = 0; i < dst.length; i++) {
            dst[i] = dst[i] << up;
        }
    }
