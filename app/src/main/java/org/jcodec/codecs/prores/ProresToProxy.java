...
public class ProresToProxy {
    ...
    private void transcodePicture(ByteBuffer inBuf, ByteBuffer outBuf, ProresConsts.FrameHeader fh) {
        ...
        for (int i = 0; i < ph.sliceSizes.length; i++) {
            ...
            int max = ((this.bitsPer1024High * sliceMbCount) >> 5) + 6;
            int low = ((this.bitsPer1024Low * sliceMbCount) >> 5) + 6;
    ...
    private void transcodeSlice(ByteBuffer inBuf, ByteBuffer outBuf, int[] qMatLuma, int[] qMatChroma, int[] scan, int sliceMbCount, int mbX, int mbY, short sliceSize, int qp) {
        ...
        int qScaleOrig = ProresDecoder.clip(inBuf.get() & 255, 1, JpegConst.APP0);
        ...
        requant(ProresDecoder.bitstream(inBuf, yDataSize), new BitWriter(outBuf), sliceMbCount << 2, ProresDecoder.scaleMat(qMatLuma, qScale), ProresDecoder.scaleMat(this.qMatLumaTo, qp), scan, mbX, mbY, 0);
        int beforeCb = outBuf.position();
        requant(ProresDecoder.bitstream(inBuf, uDataSize), new BitWriter(outBuf), sliceMbCount << 1, ProresDecoder.scaleMat(qMatChroma, qScale), ProresDecoder.scaleMat(this.qMatChromaTo, qp), scan, mbX, mbY, 1);
        int beforeCr = outBuf.position();
        requant(ProresDecoder.bitstream(inBuf, vDataSize), new BitWriter(outBuf), sliceMbCount << 1, ProresDecoder.scaleMat(qMatChroma, qScale), ProresDecoder.scaleMat(this.qMatChromaTo, qp), scan, mbX, mbY, 2);
