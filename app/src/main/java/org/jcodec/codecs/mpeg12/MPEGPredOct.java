...
public class MPEGPredOct extends MPEGPred {
    ...
    public void predictPlane(int[] ref, int refX, int refY, int refW, int refH, int refVertStep, int refVertOff, int[] tgt, int tgtY, int tgtW, int tgtH, int tgtVertStep) {
        ...
        if ((refX & 7) == 0) {
            if ((refY & 7) == 0) {
                if (safe) {
                    predictEvenEvenSafe(ref, rx2, ry, refW, refH, refVertStep, refVertOff, tgt, tgtY, tgtW >> 2, tgtH >> 2, tgtVertStep);
                    return;
                } else {
                    predictEvenEvenUnSafe(ref, rx2, ry, refW, refH, refVertStep, refVertOff, tgt, tgtY, tgtW >> 2, tgtH >> 2, tgtVertStep);
                    return;
                }
            }
            if (safe) {
                predictOddEvenSafe(ref, rx2, ry, refY - (ry << 3), refW, refH, refVertStep, refVertOff, tgt, tgtY, tgtW >> 2, tgtH >> 2, tgtVertStep);
                return;
            } else {
                predictOddEvenUnSafe(ref, rx2, ry, refY - (ry << 3), refW, refH, refVertStep, refVertOff, tgt, tgtY, tgtW >> 2, tgtH >> 2, tgtVertStep);
                return;
            }
        }
        if ((refY & 7) == 0) {
            if (safe) {
                predictEvenOddSafe(ref, rx2, refX - (rx2 << 3), ry, refW, refH, refVertStep, refVertOff, tgt, tgtY, tgtW >> 2, tgtH >> 2, tgtVertStep);
                return;
            } else {
                predictEvenOddUnSafe(ref, rx2, refX - (rx2 << 3), ry, refW, refH, refVertStep, refVertOff, tgt, tgtY, tgtW >> 2, tgtH >> 2, tgtVertStep);
                return;
            }
        }
        if (safe) {
            predictOddOddSafe(ref, rx2, refX - (rx2 << 3), ry, refY - (ry << 3), refW, refH, refVertStep, refVertOff, tgt, tgtY, tgtW >> 2, tgtH >> 2, tgtVertStep);
        } else {
            predictOddOddUnSafe(ref, rx2, refX - (rx2 << 3), ry, refY - (ry << 3), refW, refH, refVertStep, refVertOff, tgt, tgtY, tgtW >> 2, tgtH >> 2, tgtVertStep);
        }
    }
    ...
    protected int getPix4(int[] ref, int refW, int refH, int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, int refVertStep, int refVertOff, int ix, int iy) {
        ...
        int nix = 8 - ix;
        int niy = 8 - iy;
        return ((((((ref[(MathUtil.clip(y1, 0, lastLine) * refW) + MathUtil.clip(x1, 0, refW - 1)] * nix) * niy) + ((ref[(MathUtil.clip(y2, 0, lastLine) * refW) + MathUtil.clip(x2, 0, refW - 1)] * ix) * niy)) + ((ref[(MathUtil.clip(y3, 0, lastLine) * refW) + MathUtil.clip(x3, 0, refW - 1)] * nix) * iy)) + ((ref[(MathUtil.clip(y4, 0, lastLine) * refW) + MathUtil.clip(x4, 0, refW - 1)] * ix) * iy)) + 32) >> 6;
    }
    ...
    private void predictOddOddSafe(int[] ref, int rx2, int ix, int ry, int iy, int refW, int refH, int refVertStep, int refVertOff, int[] tgt, int tgtY, int tgtW, int tgtH, int tgtVertStep) {
        ...
        for (int i = 0; i < tgtH; i++) {
            ...
            while (true) {
                ...
                if (j < tgtW) {
                    offTgt2 = offTgt + 1;
                    tgt[offTgt] = ((((((ref[offRef] * nix) * niy) + ((ref[offRef + 1] * ix) * niy)) + ((ref[offRef + stride] * nix) * iy)) + ((ref[(offRef + stride) + 1] * ix) * iy)) + 32) >> 6;
                    offRef++;
                    j++;
                }
            }
    ...
    protected int getPix2(int[] ref, int refW, int refH, int x1, int y1, int x2, int y2, int refVertStep, int refVertOff, int i) {
        int ni = 8 - i;
        int lastLine = (refH - (1 << refVertStep)) + refVertOff;
        return (((ref[(MathUtil.clip(y1, 0, lastLine) * refW) + MathUtil.clip(x1, 0, refW - 1)] * ni) + (ref[(MathUtil.clip(y2, 0, lastLine) * refW) + MathUtil.clip(x2, 0, refW - 1)] * i)) + 4) >> 3;
    }
