...
public class FileSystemUtils {
    ...
    /* renamed from: DF */
    private static final String f1436DF;
    private static final int INIT_PROBLEM = -1;
    private static final FileSystemUtils INSTANCE = new FileSystemUtils();

    /* renamed from: OS */
    private static final int f1437OS;
    ...
    public static long freeSpace(String path) throws IOException {
        return INSTANCE.freeSpaceOS(path, f1437OS, false, -1L);
    }
    ...
    public static long freeSpaceKb(String path, long timeout) throws IOException {
        return INSTANCE.freeSpaceOS(path, f1437OS, true, timeout);
    }
    ...
    long freeSpaceUnix(String path, boolean kb, boolean posix, long timeout) throws IOException {
        ...
        String[] cmdAttribs = flags.length() > 1 ? new String[]{f1436DF, flags, path} : new String[]{f1436DF, path};
        List<String> lines = performCommand(cmdAttribs, 3, timeout);
        if (lines.size() < 2) {
            throw new IOException("Command line '" + f1436DF + "' did not return info as expected for path '" + path + "'- response was " + lines);
        }
        ...
        if (tok.countTokens() < 4) {
            if (tok.countTokens() == 1 && lines.size() >= 3) {
                ...
                throw new IOException("Command line '" + f1436DF + "' did not return data as expected for path '" + path + "'- check path is valid");
            }
        } else {
            tok.nextToken();
        }
    ...
    long parseBytes(String freeSpace, String path) throws IOException {
        try {
            ...
            if (bytes < 0) {
                throw new IOException("Command line '" + f1436DF + "' did not find free space in response for path '" + path + "'- check path is valid");
            }
            return bytes;
        } catch (NumberFormatException ex) {
            throw new IOExceptionWithCause("Command line '" + f1436DF + "' did not return numeric data as expected for path '" + path + "'- check path is valid", ex);
        }
    }
