...
public class XmlStreamReader extends Reader {
    ...
    private String doLenientDetection(String httpContentType, XmlStreamReaderException ex) throws IOException {
        if (httpContentType != null && httpContentType.startsWith(NetworkLog.HTML)) {
            try {
                return calculateHttpEncoding(NetworkLog.XML_2 + httpContentType.substring(NetworkLog.HTML.length()), ex.getBomEncoding(), ex.getXmlGuessEncoding(), ex.getXmlEncoding(), true);
    ...
    String calculateRawEncoding(String bomEnc, String xmlGuessEnc, String xmlEnc) throws IOException {
        if (bomEnc == null) {
            ...
            return (xmlEnc.equals(UTF_16) && (xmlGuessEnc.equals(UTF_16BE) || xmlGuessEnc.equals(UTF_16LE))) ? xmlGuessEnc : xmlEnc;
        }
    ...
    static boolean isAppXml(String mime) {
        return mime != null && (mime.equals(NetworkLog.XML_1) || mime.equals("application/xml-dtd") || mime.equals("application/xml-external-parsed-entity") || (mime.startsWith("application/") && mime.endsWith("+xml")));
    }

    static boolean isTextXml(String mime) {
        return mime != null && (mime.equals(NetworkLog.XML_2) || mime.equals("text/xml-external-parsed-entity") || (mime.startsWith("text/") && mime.endsWith("+xml")));
    }
}
